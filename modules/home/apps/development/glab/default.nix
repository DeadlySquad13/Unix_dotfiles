...
}
{
  ...
  home.file.".config/glab-cli/config_source.yml" = {
    # INFO: GlabCli requires 600 permissions on the file.
    #
    # Reference: https://stackoverflow.com/questions/76955208/nixos-home-manager-ssh-config-permissions
    onChange = ''cat ~/.config/glab-cli/config_source.yml > ~/.config/glab-cli/config.yml && chmod 600 ~/.config/glab-cli/config.yml'';
